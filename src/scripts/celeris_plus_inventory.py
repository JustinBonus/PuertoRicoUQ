...
# === Parameters ===
csv_file = "Loiza_Inventory_1m.csv"  # Change this to your actual CSV path
pixel_size = 0.000009000009*1.0 # Degrees per pixel (adjust based on data resolution)
floor_height = 3.0 # Length unit per floor
    ...
    for i in range(min_row, max_row + 1):
        for j in range(min_col, max_col + 1):
            ...
            if translated.intersects(pixel):
                raster[height - i - 1, j] = max(raster[height - i - 1, j], floors*floor_height)
